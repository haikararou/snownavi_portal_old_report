...
<body id="sub" class="report summer"><!--<body id="sub" class="report winter">-->
<div id="container">
	...
	<div id="content">
		...
		<?php if(have_posts()):while(have_posts()):the_post(); ?>
			<li>
				...
				<h5><?php the_title(); ?><span class="tag"><?php $terms = wp_get_object_terms($post->ID,'entrekking'); foreach($terms as $term){echo $term->name . ' ';} ?></span></h5>
...
<?php get_sidebar("skidata"); ?>
	...
	<?php else : ?>	<!-- アーカイブのとき -->		
				<ul id="tab">
					<li class="tab01"><a href="<?php echo home_url();?>/enskireport/en/"><span>Ski Area Report</span><i class="fa fa-caret-down"></i></a></li>
					<li class="tab02"><a href="<?php echo home_url();?>/entrekking/en/" class="select"><span>Trekking Report</span><i class="fa fa-caret-down"></i></a></li>
				</ul>
					...
					<?php if(have_posts()):while(have_posts()):the_post(); ?>
						<li class="post">
							<a href="<?php echo the_permalink();echo "?lang=en"; ?>">
								<p class="tag"><span><?php $terms = wp_get_object_terms($post->ID,'trekking'); foreach($terms as $term){echo $term->slug . ' ';} ?></span></p>
